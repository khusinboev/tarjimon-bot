    ...
    if message.text == '0000':
        ...
        for row in rows:
            ...
            try:
                ...
                db.commit()
                sql.execute(f"DELETE FROM public.user_langs WHERE user_id ='{id}'")
                db.commit()
                sql.execute(f"DELETE FROM public.users_status WHERE user_id ='{id}'")
                db.commit()
                sql.execute(f"DELETE FROM public.users_tts WHERE user_id ='{id}'")
                db.commit()
            except ChatNotFound:
                sql.execute(f"DELETE FROM public.accounts WHERE user_id ='{id}'")
                db.commit()
