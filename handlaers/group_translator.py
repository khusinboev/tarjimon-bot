...
async def helper(message: types.Message):
    try:
        # await message.answer("Bot starting in 5 minutes")
...
async def Check(message: types.Message):
    ...
    return message.reply_to_message['from']['username'] == info.username
...
@dp.callback_query_handler(text=CallFilter("all"), chat_type=[types.ChatType.GROUP, types.ChatType.SUPERGROUP])
...
@dp.message_handler(lambda message: Check(message), content_types=types.ContentTypes.TEXT,
...
async def group_handle(message: types.Message):
    ...
    # except:
    #     await message.answer("/start /lang")
